...
public class FriendsResource {
    ...
    @SeeAlso({@See(href = "/docs/api/1.1/get/followers/ids", value="GET followers/ids"),@See(href = "/docs/api/1.1/get/followers/list", value="GET followers/list"),@See(href = "/docs/api/1.1/get/friends/list", value="GET friends/list"),@See(href = "/docs/misc/cursoring", value="Using cursors to navigate collections")})
    @ApplicationLimit(rate = @Rate(window = 15, unit = GovernanceUnit.MINUTES, limit = 15))@UserLimit(rate = @Rate(window = 15, unit = GovernanceUnit.MINUTES, limit = 15))@Description("Returns a cursored collection of user IDs for every user the specified user is following (otherwise known as their ")
    ...
    @ApiVersion({"1.1"})
    @Tag({"Bulk Operations","finding users"})
    @SeeAlso({@See(href = "/docs/api/1.1/get/friends/ids", value="GET friends/ids"),@See(href = "/docs/api/1.1/get/followers/ids", value="GET followers/ids")})
    @ApplicationLimit(rate = @Rate(window = 15, unit = GovernanceUnit.MINUTES, limit = 30))@UserLimit(rate = @Rate(window = 15, unit = GovernanceUnit.MINUTES, limit = 15))@Description("Returns a cursored collection of user objects for every user the specified user is following (otherwise known as their ")
