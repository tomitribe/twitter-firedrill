...
public class SearchResource {
    ...
    @SeeAlso({@See(href = "/docs/api/1.1/get/users/search", value="GET users/search"),@See(href = "/docs/using-search", value="Using the Twitter Search API"),@See(href = "/docs/api/1.1/get/geo/search", value="GET geo/search"),@See(href = "/docs/finding-tweets-about-places", value="Finding Tweets about Places")})
    @ApplicationLimit(rate = @Rate(window = 15, unit = GovernanceUnit.MINUTES, limit = 450))@UserLimit(rate = @Rate(window = 15, unit = GovernanceUnit.MINUTES, limit = 180))@Description("Returns relevant tweets that match a specified query.")
    public TweetsType getTweets(@Description("A UTF-8, URL-encoded search query of 1,000 characters maximum, including operators. Queries may additionally be limited by complexity.")
     @PathParam("q") final String q, @Description("Returns tweets by users located within a given radius of the given latitude/longitude. The location is preferentially taking from the Geotagging API, but will fall back to their Twitter profile. The parameter value is specified by &quot;")
     ...
     @QueryParam("locale") final String locale, 
                @Description("Optional. Specifies what type of search results you would prefer to receive. The current default is &quot;mixed.&quot; Valid values include:")
