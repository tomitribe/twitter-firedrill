...
public class SavedsearchesResource {
    ...
    @ApplicationLimit()@UserLimit(rate = @Rate(window = 15, unit = GovernanceUnit.MINUTES, limit = 15))@Description("Returns the authenticated user&amp;#039;s saved search queries.")
    ...
    @ApplicationLimit()@UserLimit(rate = @Rate(window = 15, unit = GovernanceUnit.MINUTES, limit = 15))@Description("Retrieve the information for the saved search represented by the given id. The authenticating user must be the owner of saved search ID being requested.")
